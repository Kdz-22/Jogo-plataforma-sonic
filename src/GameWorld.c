...
void updateGameWorld(GameWorld *gw, float delta) {
  ...
  if (gw->estado == ESTADO_JOGO_JOGANDO) {
    ...
    gw->personagem->funcoes->entrada(gw->personagem->dados, gw->personagem,
                                     delta);
    gw->personagem->funcoes->atualizar(gw->personagem->dados, gw->personagem,
                                       gw, delta);
...
void drawGameWorld(GameWorld *gw) {
  ...
  if (gw->estado == ESTADO_JOGO_JOGANDO) {
    ...
    DrawTexturePro(rm.texturaHUD, (Rectangle){24, 432, 40, 16},
                   (Rectangle){20, 15, 80, 32}, (Vector2){0, 0}, 0.0f, WHITE);
    // Time
    DrawTexturePro(rm.texturaHUD, (Rectangle){24, 456, 32, 16},
                   (Rectangle){20, 45, 64, 32}, (Vector2){0, 0}, 0.0f, WHITE);
    // Rings
    DrawTexturePro(rm.texturaHUD, (Rectangle){24, 480, 40, 16},
                   (Rectangle){20, 75, 80, 32}, (Vector2){0, 0}, 0.0f, WHITE);
    ...
    DrawTexturePro(rm.texturaHUD, (Rectangle){40, 400, 16, 16},
                   (Rectangle){posVidas.x, posVidas.y, 32, 32}, (Vector2){0, 0},
                   0.0f, WHITE);
    // Texto "Sonic"
    ...
    DrawText("GAME OVER", larguraTela / 2 - tamTexto1 / 2, alturaTela / 2 - 40,
             40, RED);

    int tamTexto2 = MeasureText("PRESSIONE 'R' PARA REINICIAR", 20);
    DrawText("PRESSIONE 'R' PARA REINICIAR", larguraTela / 2 - tamTexto2 / 2,
             alturaTela / 2 + 20, 20, LIGHTGRAY);
  }
  EndDrawing();
}
...
static void inicializar(GameWorld *gw) {
  ...
  gw->personagem = criarSonic(GetScreenWidth() / 2 + 144,
                              calcularAlturaMapa(gw->mapa) - 196, 96, 96);

  gw->camera = (Camera2D){
      .offset = {0}, // deslocamento relativo da câmera em relação ao alvo
      .target = {0}, // o alvo da câmera, ou seja, a coordenada em que ela está
                     // centralizada
...
static void desenharTempo(Texture2D hud, int tempo, Vector2 posicao) {
  ...
  for (int i = 0; buffer[i] != '\0'; i++) {
    ...
    if (buffer[i] == ':') {
      DrawRectangle(posicao.x + i * (digitoW * 2) + 6, posicao.y + 8, 4, 4,
                    WHITE);

      DrawRectangle(posicao.x + i * (digitoW * 2) + 6, posicao.y + 20, 4, 4,
                    WHITE);
