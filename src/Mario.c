...
static void desenharQuadroAnimacaoJogador( Mario *m, QuadroAnimacao *qa, Color tonalidade );
static QuadroAnimacao *getQuadroAnimacaoAtualMario( Mario *m );
static Animacao *getAnimacaoAtualMario( Mario *m );

static void resolverColisaoJogadorObstaculosMapaX( Mario *m, Mapa *mapa );
static void resolverColisaoJogadorObstaculosMapaY( Mario *m, Mapa *mapa );

static void resolverColisaoJogadorItensMapa( Mario *m, Personagem *p, Mapa *mapa );
static void resolverColisaoJogadorInimigosMapa( Mario *m, Personagem *p, Mapa *mapa );
...
void entradaMario( void *dados, Personagem *p, float delta ) {
    ...
    if ( direitaDown ) {
        if ( m->vel.x < 0 ) {
            ...
            if ( !m->freando && m->estado == ESTADO_JOGADOR_CORRENDO ) {
    ...
    if ( m->quantidadePulos > 0 ) {
        if ( absVelX <= m->velAndando ) {
            m->estado = ESTADO_JOGADOR_PULANDO;
        } else {
            m->estado = ESTADO_JOGADOR_PULANDO_CORRENDO;
        }
    } else if ( absVelX < 1.0f ) {
        m->estado = ESTADO_JOGADOR_PARADO;
    } else if ( absVelX <= m->velAndando ) {
        m->estado = ESTADO_JOGADOR_ANDANDO;
    } else {
        m->estado = ESTADO_JOGADOR_CORRENDO;
    }
    ...
    /*if ( estadoAnterior == ESTADO_JOGADOR_ANDANDO && m->estado == ESTADO_JOGADOR_ANDANDO_RAPIDO ) {
         sincronizarAnimacao( &m->animacaoAndandoRapido, &m->animacaoAndando );
    } else if ( estadoAnterior == ESTADO_JOGADOR_ANDANDO_RAPIDO && m->estado == ESTADO_JOGADOR_ANDANDO ) {
...
void atualizarMario( void *dados, Personagem *p, GameWorld *gw, float delta ) {
    ...
    if ( m->estado < ESTADO_JOGADOR_PULANDO ) {
        p->comboAereo = 0;
    }
    ...
    resolverColisaoJogadorObstaculosMapaX( m, gw->mapa );
    ...
    resolverColisaoJogadorObstaculosMapaY( m, gw->mapa );

    resolverColisaoJogadorItensMapa( m, p, gw->mapa );
    resolverColisaoJogadorInimigosMapa( m, p, gw->mapa );
    ...
    while ( el != NULL ) {
        ...
        if ( inimigo->tipo == TIPO_INIMIGO_MOTOBUG ) {
            ...
            if ( CheckCollisionRecs( retColCalculado, retColInimigoCalculado ) ) {
                ...
                if ( m->estado >= ESTADO_JOGADOR_PULANDO && m->estado <= ESTADO_JOGADOR_PULANDO_CORRENDO ) {
                    m->vel.y = m->velPulo;
...
Personagem *criarMario( float x, float y, float w, float h ) {
    ...
    criarQuadrosAnimacao( &m->animacaoParado, m->animacaoParado.quantidadeQuadros );
    ...
    m->animacaoAndando.quantidadeQuadros = 8;
    ...
    criarQuadrosAnimacao( &m->animacaoAndando, m->animacaoAndando.quantidadeQuadros );
    inicializarQuadrosAnimacao( 
    ...
    m->animacaoCorrendo.quantidadeQuadros = 4;
    ...
    criarQuadrosAnimacao( &m->animacaoCorrendo, m->animacaoCorrendo.quantidadeQuadros );
    inicializarQuadrosAnimacao( 
    ...
    m->animacaoPulando.quantidadeQuadros = 4;
    ...
    criarQuadrosAnimacao( &m->animacaoPulando, m->animacaoPulando.quantidadeQuadros );
    inicializarQuadrosAnimacao( 
    ...
    m->animacaoPulandoCorrendo.quantidadeQuadros = 4;
    ...
    criarQuadrosAnimacao( &m->animacaoPulandoCorrendo, m->animacaoPulandoCorrendo.quantidadeQuadros );
    inicializarQuadrosAnimacao(
    ...
    m->animacoes[ESTADO_JOGADOR_PARADO] = &m->animacaoParado; quantidadeAnimacoes++;
    m->animacoes[ESTADO_JOGADOR_ANDANDO] = &m->animacaoAndando; quantidadeAnimacoes++;
    m->animacoes[ESTADO_JOGADOR_CORRENDO] = &m->animacaoCorrendo; quantidadeAnimacoes++;
    m->animacoes[ESTADO_JOGADOR_PULANDO] = &m->animacaoPulando; quantidadeAnimacoes++;
    m->animacoes[ESTADO_JOGADOR_PULANDO_CORRENDO] = &m->animacaoPulandoCorrendo; quantidadeAnimacoes++;
